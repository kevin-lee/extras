...
/** @author Kevin Lee
  * @since 2023-10-16
  */
object MkPascalCaseStringSpec extends Properties {
  ...
  def testMkPascalCaseStringWithAllUpperCases: Property =
    for {
      ...
      input          <- Gen.constant(allPascalCases.map(_.toUpperCase())).log("input")
    } yield {
  ...
  def testMkPascalCaseStringWithAllLowerCases: Property =
    for {
      ...
      input          <- Gen.constant(allPascalCases.map(_.toLowerCase())).log("input")
    } yield {
      val expected = allPascalCases.mkString
      val actual   = input.mkPascalCaseString
      actual ==== expected
